...
describe('base:core.dom', () => {
  describe('ancestor', () => {
    ...
    before(() => {
      // basic case
      $cont = $(
        '<div class="note-editable"><b>b</b><u>u</u><s>s</s><i>i</i></div>',
      ); // busi
      $b = $cont.find('b');
      txtB = $b[0].firstChild;
    });
  ...
  describe('listAncestor', () => {
    ...
    before(() => {
      $cont = $(
        '<div class="note-editable"><i><s><u><b>b</b></u></s></i></div>',
      ); // busi
      $b = $cont.find('b');
      $u = $cont.find('u');
      $s = $cont.find('s');
  ...
  describe('listDescendant', () => {
    ...
    before(() => {
      $cont = $(
        '<div class="note-editable"><b></b><u></u><s></s><i></i></div>',
      ); // busi
      $b = $cont.find('b');
      $u = $cont.find('u');
      $s = $cont.find('s');
    ...
    it('should return an array of descendant elements', () => {
      expect(dom.listDescendant($cont[0])).to.deep.equal([
        $b[0],
        $u[0],
        $s[0],
        $i[0],
  ...
  describe('makeOffsetPath', () => {
    ...
    it('should return offset path array between two elements #1', () => {
      ...
      expect(dom.makeOffsetPath($cont[0], $b[0].firstChild)).to.deep.equal([
        0, 0,
      ]);
    });
    ...
    it('should return offset path array between two elements #2', () => {
      ...
      expect(dom.makeOffsetPath($cont[0], $u[0].firstChild)).to.deep.equal([
        1, 0,
      ]);
    });
    ...
    it('should return offset path array between two elements #3', () => {
      ...
      expect(dom.makeOffsetPath($cont[0], $s[0].firstChild)).to.deep.equal([
        2, 0,
      ]);
    });
    ...
    it('should return offset path array between two elements #2', () => {
      ...
      expect(dom.makeOffsetPath($cont[0], $i[0].firstChild)).to.deep.equal([
        3, 0,
      ]);
    });
  });
  ...
  describe('fromOffsetPath', () => {
    ...
    it('should return the element by offsetPath', () => {
      ...
      $.each([$b[0], $u[0], $s[0], $i[0]], (idx, node) => {
        expect(
          dom.fromOffsetPath(cont, dom.makeOffsetPath(cont, node)),
        ).to.deep.equal(node);
        let child = node.firstChild;
        expect(
          dom.fromOffsetPath(cont, dom.makeOffsetPath(cont, child)),
        ).to.deep.equal(child);
      });
    });
  });
  ...
  describe('splitTree', () => {
    ...
    beforeEach(() => {
      let $busi = $(
        '<div class="note-editable"><p><b>b</b><u>u</u><s>strike</s><i>i</i></p></div>',
      ); // busi
    ...
    describe('textNode case', () => {
      ...
      it('should be split by span tag with offset 2 (1 depth and element case)', () => {
        let $cont = $(
          '<div class="note-editable"><p><span><b>b</b><u>u</u><s>s</s><i>i</i></span></p></div>',
        ); // busi
        let $span = $cont.find('span');
        dom.splitTree($span[0], { node: $span[0], offset: 2 });

        expect($cont.html()).to.equalsIgnoreCase(
          '<p><span><b>b</b><u>u</u></span><span><s>s</s><i>i</i></span></p>',
        );
      });
    });
  });
