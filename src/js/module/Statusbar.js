...
export default class Statusbar {
  ...
  initialize() {
    ...
    this.$statusbar.on("mousedown", (event) => {
      ...
      const editableCodeTop =
        this.$codable.offset().top - this.$document.scrollTop();
      ...
      const onMouseMove = (event) => {
        ...
        heightCode =
          this.options.minheight > 0
            ? Math.max(heightCode, this.options.minheight)
            : heightCode;
        heightCode =
          this.options.maxHeight > 0
            ? Math.min(heightCode, this.options.maxHeight)
            : heightCode;
